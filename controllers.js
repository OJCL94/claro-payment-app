...
class ClaroPaymentController {
    ...
    /**
     * Sets up handlers for the voucher preview view
     */
    setupVoucherView() {
        ...
        // Set voucher amount
        if (voucherAmount) {
            voucherAmount.textContent = this.model.getTotalAmount().toFixed(2);
        }
        
        // Generate a random customer name if not set
        if (customerName && !customerName.textContent) {
            const names = ['JUAN DIAZ', 'MARIA LOPEZ', 'CARLOS RODRIGUEZ', 'ANA GARCIA'];
            customerName.textContent = names[Math.floor(Math.random() * names.length)];
        }
